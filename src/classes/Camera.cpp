...
const Camera::ClassDefinition Camera::DEFINITION{
    ...
    .Properties = {
        USERDATA_READWRITE_PROP(Camera, CFrame, gargantuan::CFrame),
        USERDATA_READWRITE_PROP(Camera, FieldOfView, float),
        USERDATA_READWRITE_PROP(Camera, ViewportSize, gargantuan::Vector2),
        {
...
float Camera::GetAspectRatio() { return ViewportSize.GetY() > 0.0f ? ViewportSize.GetX() / ViewportSize.GetY() : 1.0f; }
