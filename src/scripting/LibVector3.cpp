...
int Vector3_Lerp(lua_State *L) {
    ...
    lua_pushvector(
        L, self.x + (other.x - self.x) * alpha, self.y + (other.y - self.y) * alpha, self.z + (other.z - self.z) * alpha
    );
    return 1;
}
...
std::unordered_map<std::string_view, std::function<int(lua_State *)>> Vector3_methods = {
    ...
    {"Cross", Vector3_Cross}, {"Angle", Vector3_Angle}, {"Dot", Vector3_Dot},     {"FuzzyEq", Vector3_FuzzyEq},
    {"Lerp", Vector3_Lerp},   {"Min", Vector3_Min},     {"Max", Vector3_Max},
};
