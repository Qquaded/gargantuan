...
template <> struct StackValue<std::string_view> {
    static inline std::string_view ReflectedTypedef() { return "string"; };
    static bool Is(lua_State *L, int idx) { return lua_isstring(L, idx); };
    static std::string_view From(lua_State *L, int idx) {
        size_t len;
        const char *str = luaL_checklstring(L, idx, &len);
        return {str, len};
    };
    static void Push(lua_State *L, std::string_view value) { return lua_pushlstring(L, value.data(), value.length()); };
};
